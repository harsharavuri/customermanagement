function getRow(slno) {
    var row =
        ...
        '<td>' + slno.toString() +'.</td>' +
        '<td>'+
        '<input name="userid" class="participant-id form-control" type="text" placeholder="Enter Spree-ID">' +
        '</td>' +
...
function addRow(e) {
    ...
    if ($('.participant-id').length < $("#spotlight-selected option:selected").data('max')) {
        var numTeamMembers = $('.participant').length+1;
        var newRow = getRow(numTeamMembers);
        $("#participants tbody").append(newRow);
        return false;
    } else {
        alert("Maximum team size: " + $("#spotlight-selected option:selected").data('max') + "reached");
        return false;
    }
}
...
function updateCost(e) {
    var workshopCount = 0;
    var cost = 0;
    cost += $("#spotlight-selected option:selected").data("cost");
    $("#spotlights-cost").html(cost);
}
...
function submitTeam() {
    var r = confirm("Make sure you took " + $("#spotlights-cost").html() + " Rs");
    if(r === false)
    return false;
    //alert("Make sure you took " + $("#total-cost").html() + " Rs");

    var total = $('.participant-id').length;
    if (total < $("#spotlight-selected option:selected").data('min') || total > $("#spotlight-selected option:selected").data('max')) {
...
function showWorkshopDetails(e) {
    var selectedElement = $("#spotlight-selected").children("option:selected");
    $("#spotlight-details").html("Minimum number of team members: " +
...
$(function() {
    ...
    showWorkshopDetails();
    $("#spotlight-selected").change(workshopSelectionWrapper);
    updateCost();
});