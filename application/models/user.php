...
class User extends CI_Model {
    ...
    public function get_cities(){
        $this->db->select('*')->from('city_lookup');
        $query=$this->db->get();
        if($query->num_rows>0){
            return $query->result();
        }else{
    ...
    public function get_teams_spotlights($userid)
    {
        $this->db->select('teamid')->where('userid', $userid);
        $query = $this->db->get('userspotlights',1);
        return $query->result_array();
    }
    ...
    //changed hehre
    public function in_spotlight($userid, $workshopid)
    {
        $query = $this->db->get_where('userspotlights', array('userid' => $userid, 'sid' => $workshopid));
        return $query->num_rows();
    }
    ...
    public function add_team_spotlights($userid, $workshopid, $teamid)
    {
        return $this->db->insert('userspotlights',
            array("userid" => $userid,
                "teamid" => $teamid,
                "sid" => $workshopid));
    }
    ...
    public function add_proshows($userid)
    {
        $data['proshows'] = 1;
        return $this->db->update('userdata', $data, array('userid' => $userid));
    }
